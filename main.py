...
def draw_pixel_with_smooth(graph, x, y, color, opacity):
    current_color = graph.get_pixel_color(x, y)  # Получаем текущий цвет пикселя
    smoothed_color = interpolate_color(current_color, color, opacity)  # Вычисляем сглаженный цвет

    draw_pixel(graph, x, y, smoothed_color)  # Рисуем пиксель с сглаженным цветом
